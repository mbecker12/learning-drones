...
class Sensor:
    ...
    def verlet_get_delta_v(self):
        delta_v = (
            0.5 * (self.last_acceleration + self.current_acceleration) * self.delta_t
        )
        return delta_v
...
class DataGenerator:
    ...
    def __init__(self, noise=0.0, method=None, **kwargs):
...
if __name__ == "__main__":
    ...
    ax0.plot(
        x[0]
        + v[0] * (dt * time_grid)
        + 0.5 * acc * (dt * time_grid) * (dt * time_grid),
        label="sq",
    )
    ax0.legend()
    ...
    for t in time_grid:
        if t == n - 1:
            break
        s.measure_acceleration(
            np.sin(omega * time_grid[t] + np.random.normal(0.0, 0.0))
        )
        a[t + 1] = s.return_acceleration()
        dv = s.naive_get_delta_v()
        # x[t+1], v[t+1] = s.get_current_x_and_v(x[t], v[t])
    ...
    ax2.plot(-dt / omega * np.cos(omega * time_grid) + dt / omega, label="v_theo")
    ax2.plot(
        dt * dt / omega * time_grid
