...
roll_target, pitch_target, yaw_target = 0 * np.pi/180, 60 * np.pi/180, 0 * np.pi/180
...
def update_sensors(sensor: [Sensor], accelerations: np.ndarray) -> (np.ndarray, np.ndarray):
...
# initialization PID
rot_pids = [
    PID(kp=0.7, ki=0.01, kd=0.1, timeStep=delta_time, setValue=roll_target, integralRange=2,
        calculateFlag="signChange", outputLimitRange=[-np.pi/4, np.pi/4]),
    PID(kp=0.7, ki=0.01, kd=0.1, timeStep=delta_time, setValue=pitch_target, integralRange=2,
        calculateFlag="signChange", outputLimitRange=[-np.pi/4, np.pi/4]),
    PID(kp=0.7, ki=0.001, kd=0.1, timeStep=delta_time, setValue=yaw_target, integralRange=2,
        calculateFlag="signChange", outputLimitRange=[-np.pi/6, np.pi/6])
]

lin_pids = [
    PID(kp=0.9, ki=0.1, kd=0.8, timeStep=delta_time, setValue=x_target, integralRange=2, calculateFlag="signChange",
        outputLimitRange=[-np.pi/4, np.pi/4]),
    PID(kp=0.9, ki=0.1, kd=0.8, timeStep=delta_time, setValue=y_target, integralRange=2, calculateFlag="signChange",
        outputLimitRange=[-np.pi/4, np.pi/4]),
    PID(kp=0.8, ki=0.1, kd=0.6, timeStep=delta_time, setValue=z_target, integralRange=2, calculateFlag="signChange")
]

# initialization Sensor
sensors = [Sensor(delta_time, initial_pos[i, 0], initial_vel[i, 0]) for i in range(3)]
sensors.extend([Sensor(delta_time, initial_angle[i, 0], initial_angle_vel[i, 0]) for i in range(3)])

# initialization Quadcopter
quadcopter = QuadcopterPhysics(mass_center=mass_center, mass_motor=mass_motor, radius_motor_center=radius_motor_center,
                               I_x=I_x, I_y=I_y, I_z=I_z,
                               coef_force=coef_force, coef_moment=coef_moment, coef_wind=coef_wind, gravity=gravity,
                               mass_payload=mass_payload, x_payload=x_payload, y_payload=y_payload
)

# initialize data scraper
dh = DataHandler(parentfolder="results", visualize=visualize, n_servers=2, port=65432, printouts=False)
...
for time in range(1000):
    ...
    # update sensors
    lab_pos, drone_angle = update_sensors(sensor=sensors, accelerations=np.concatenate([lab_lin_acc, lab_rot_acc]))
    ...
    # save data
    dh.new_data(time=time * delta_time, rotation=drone_angle, translation=lab_pos, thrusters=thrust,
                wind=wind_speed, pid=pid_out)

    print("Roll: {:.2f}, Pitch: {:.2f}, Yaw: {:.2f}".format(drone_angle[0, 0] * 180 / np.pi,
