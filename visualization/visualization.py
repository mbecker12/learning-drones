...
class Plotter:
    ...
    def __init__(
        self, host: str, port: int, printouts: bool, showing: str, n_last_states: int
    ):
        ...
        if showing == "rotations":
            ...
            # init angle plots
            self.roll_plot, roll_sp = self._setup_lineplot(
                grid=grid,
                row=0,
                start_column=2,
                end_column=4,
            ...
            pitch_plot, pitch_sp = self._setup_lineplot(
                grid=grid,
                row=1,
                start_column=2,
                end_column=4,
            ...
            yaw_plot, yaw_sp = self._setup_lineplot(
                grid=grid,
                row=2,
                start_column=2,
                end_column=4,
            ...
            # init wind plot
            wind_plot = self._setup_lineplot(
                grid=grid,
                row=3,
                start_column=2,
                end_column=4,
            ...
            self.thruster_handle = self.thruster_plot.bar(
                [0.5, 1.5, 2.5, 3.5], [0.0, 0.0, 0.0, 0.0], color="b"
            )
            ...
            self.plots.update(
                {"roll": self.roll_plot, "pitch": pitch_plot, "yaw": yaw_plot}
            )
            ...
            self.plane_plot, self.wind_text, self.setpoint_xy = self._setup_gridplot(
                grid=grid, target=(90, 80), size=plot_size
            )
            xy = (40, -55)
            dxy = (20, 0)
            self.wind_arrow_handle = self.plane_plot.arrow(
                *xy,
                *dxy,
                color="y",
            ...
            self.direction_arrow_handle = self.plane_plot.arrow(
                ...
                0,
                color="g",
                head_width=int(plot_size / 15),
                width=2,
                head_starts_at_zero=True,
            )
            self.position_handle = self.plane_plot.scatter(0, 0, c="g")
            (self.position_history_handle,) = self.plane_plot.plot(
                self.translation[:, 0], self.translation[:, 1], c="r", alpha=0.5
            )

            # init height plot
            height_plot, height_sp = self._setup_lineplot(
                grid=grid,
                row=3,
                start_column=2,
                end_column=4,
            ...
            raise TypeError(
                "There is no flag named: ",
                showing,
                " ! Please input rotations or translations",
            )
        ...
        try:
            ...
            if "quit" in received:
                ...
                if message_type:
                    ...
                    self._store_new_data(rotation=rot, translation=trans, wind=wind)
                else:
    ...
    def _setup_gridplot(
        self, grid, target: tuple, size: int
    ) -> (plt.axes, plt.text, None):
        ...
        ax.add_artist(circ)
        texter = ax.text(
            self.arrow_center[0] - self.arrow_length,
            self.arrow_center[1] + self.arrow_length,
            "Windspeed: 0 m/s",
        )
    ...
    def _setup_lineplot(
        ...
        ax.set(ylabel=label)
        y_ticks = np.arange(
            range_lower, range_upper + 1, int((range_upper - range_lower) / 2)
        )
    ...
    def _update_rotations(self, time: float, thrusters: tuple):
        ...
        self.thruster_handle = self.thruster_plot.bar(
            [0.5, 1.5, 2.5, 3.5], thrusters, color="b"
        )
    ...
    def _return_rotation(self):
        return (
            ...
            self.yaw_handle,
            self.wind_handle,
            self.thruster_handle,
        )

    def _setpoints_rotation(self, info):
        self.setpoint_handler["roll"].remove()
        self.setpoint_handler["roll"] = self.plots["roll"].axhline(
            y=info[0], lw=1, c="r"
        )
        self.setpoint_handler["pitch"].remove()
        self.setpoint_handler["pitch"] = self.plots["pitch"].axhline(
            y=info[1], lw=1, c="g"
        )
    ...
    def _update_translations(self, time: float, thrusters: tuple):
        ...
        self.wind_text.set_text(
            "Windspeed: {:.2f} m/s".format(np.linalg.norm(self.wind[-1, :]))
        )
        self.wind_arrow_handle.remove()
        self.wind_arrow_handle = self.plane_plot.arrow(
            *xy, *dxy, color="y", head_width=6, width=2, head_starts_at_zero=True
        )
        ...
        dxy = (
            np.sin(self.rotation[-1, 2] * np.pi / 180) * self.arrow_length / 2,
            np.cos(self.rotation[-1, 2] * np.pi / 180) * self.arrow_length / 2,
        )
        self.direction_arrow_handle = self.plane_plot.arrow(
            *xy, *dxy, color="g", head_width=6, width=2
        )
        self.position_handle.set_offsets(self.translation[-1, :2])
        self.position_history_handle.set_data(
    ...
    def _return_translation(self):
        return (
            self.thruster_handle,
            self.direction_arrow_handle,
            self.position_handle,
            self.position_history_handle,
            self.wind_arrow_handle,
            self.wind_text,
            self.height_handle,
        )
    ...
    def _setpoints_translation(self, info):
        ...
        self.setpoint_handler["height"] = self.plots["height"].axhline(
            y=info[3], lw=1, c="b"
        )
    ...
    def _decode_message(self, message: str):
        ...
        if meaning[0] == "SETPOINTS":
            ...
            return (
                False,
                (roll * 180 / np.pi, pitch * 180 / np.pi, yaw * 180 / np.pi, z, (x, y)),
            )
        else:
            try:
                (
                ...
                return (
                    ...
                    (
                        time,
                        np.array([[roll, pitch, yaw]]) * 180 / np.pi,
                        np.array([[x, y, z]]),
                        thrust,
    ...
    # OTHER
    def _store_new_data(
        self, rotation: np.ndarray, translation: np.ndarray, wind: float
    ):
